## Test dataset for normality using Levene and Bartlett test
# Load Libraries
...
bart.soil <- matrix (data = NA, ncol = 13, nrow = 1)
colnames (bart.soil) <- colnames(chem[,8:20])
rownames(bart.soil) <- "bar.soil.prenorm"

for (i in 8:20) 
{
  b <- bartlett.test(chem[, i] ~ Ecotype, data = chem)
  bart.soil[1,i-7] <- as.numeric(b$p.value)
}

#### Levene's test for soil data ####
lev.soil <- matrix (data = NA, ncol = 13, nrow = 1)
colnames (lev.soil) <- colnames(chem[,8:20])
rownames(lev.soil) <- "lev.soil.prenorm"

for (i in 8:20) 
{
  l <- leveneTest(chem[, i] ~ Ecotype, data = chem)
  lev.soil[1,i-7] <- as.numeric(l[[3]][1])
}
...
bart.soil.nm <- matrix (data = NA, ncol = 13, nrow = 1)
colnames (bart.soil.nm) <- colnames(soil.norm[,7:19])
...
lev.soil.nm <- matrix (data = NA, ncol = 13, nrow = 1)
colnames (lev.soil.nm) <- colnames(soil.norm[,7:19])
