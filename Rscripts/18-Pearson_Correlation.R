##Pearson correlations for plant metal uptake, Cd & Zn content
## Load libraries
library(ggplot2)
library(dplyr)
library(ggpubr)
...
pl14 <- plant %>%
  filter(plant$Population_type == "NM_PL14_P")

pl35 <- plant %>%
  filter(plant$Population_type == "NM_PL35_P")

pl22 <- plant %>%
  filter(plant$Population_type == "M_PL22_P")

pl27 <- plant %>%
  filter(plant$Population_type == "M_PL27_P")
...
pl14.cor <- cor.test(pl14$Zn_plant, pl14$Cd_plant, 
    method = "pearson")

pl35.cor <- cor.test(pl35$Zn_plant, pl35$Cd_plant, 
                     method = "pearson")

pl22.cor <- cor.test(pl22$Zn_plant, pl22$Cd_plant, 
                     method = "pearson")

pl27.cor <- cor.test(pl27$Zn_plant, pl27$Cd_plant, 
                     method = "pearson")
